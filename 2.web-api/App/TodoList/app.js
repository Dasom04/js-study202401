...
function makeNewTodoNode(newTodo) {
  ...
  // 클래스 이름을 두 개 이상 add 하실 때는 각각 지정해야 합니다.
  // 한번에 공백 포함 두 개 이상 설정하면 에러입니다.
  //$modIcon.classList.add('lnr inr-undo'); (X)
  ...
  $remIcon.classList = 'lnr lnr-cross-circle';
...
function makeNewId() {
  if(todos.length > 0) {
    // 배열 내의 할일 ㅐㄱ체 중 마지막 객체의 id보다 하나 크게,
...
function insertToDoData(){
  ...
  //2. todos 배열에 객체를 생성한 후 추가 (id 추가 순서대로 잘 진행하세요.) push , log, id 추가 할 일이 하나도 없다면 도 고려
  const newTodo = {
    id: makeNewId(),
    text: $todoText.value,
    done: false
  ...
  //makeNewToDoList(2번에서 생성한 할 일 객채를 전달);
...
// data-id 값으로 배열을 탐색하여 일치하는 객체가 들어잇는 인덱스 변환.
function finIndexByDataId(dataId) {
...
function changeCheckState($label) {
  ...
  */
  ...
  const index = finIndexByDataId(dataId);
...
function removeTodoData($delTarget) {
  ...
  // 함수화 시켜보세요.
  const index = finIndexByDataId(+$delTarget.dataset.id);
...
function enterModifyMode($modspan) {
  ...
  // $modInput.setAttribute('type', 'text'); (ㅇ) 인풋 텍스트가 텍스트가 아니면 꼭 써줘야함
...
function modifyTodoData($modCompletespan) {
  ...
  const idx = finIndexByDataId($label.parentNode.dataset.id);
...
(function(){
  ...
  $addBtn.addEventListener('click', e =>{
    ...
    // form태그 안의 button은 type을 명시하지 않으면 자동 sumbit이 동작합니다.
    e.preventDefault(); // 버튼의 고유 기능(sumbit)을 중지.
